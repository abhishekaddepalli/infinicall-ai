...
exports.getPhoneNumbers = async (req, res) => {
  try {
    ...
    const numbers = await PhoneNumber.find(query)
      .populate('agent_id', 'name telephony_provider')
      .populate('sip_trunk_id', 'name engine provider sip_host')
      .sort(sortOptions)
      .lean();

    const campaigns = await Campaign.find({ phoneNumberId: { $in: numbers.map(n => n._id) } }, 'phoneNumberId').lean();
    const usedNumberIds = new Set(campaigns.map(c => c.phoneNumberId.toString()));
...
exports.updatePhoneNumber = async (req, res) => {
  try {
    ...
    if (updateData.agent_id) {
      ...
      const existCampaign = await Campaign.findOne({ phoneNumberId: id });
      if (existCampaign) {
        return res.status(400).json({
          success: false,
      ...
      if (agent.telephony_provider === 'elevenlabs_sip' && !elevenlabsAgentId) {
