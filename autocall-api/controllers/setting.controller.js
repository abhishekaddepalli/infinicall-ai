...
exports.updateSettings = async (req, res) => {
  try {
    ...
    const aiFields = ['openai_api_key', 'elevenlabs_api_key', 'gemini_api_key', 'anthropic_api_key', 'twilio_account_sid', 'twilio_auth_token', 'sarvam_api_key', 'plivo_auth_id', 'plivo_auth_token', 'plivo_phone_number', 'currency_symbol', 'currency_code'];
    ...
    const allFields = [
      ...
      ...chatFields, ...subscriptionFields, ...paymentFields, ...storageFields, ...aiFields, ...demoFields, ...widgetFields, ...creditFields, ...agreementFields
    ];
    ...
    allFields.forEach((field) => {
      if (req.body[field] !== undefined) {
        if (field === 'maintenance_allowed_ips' || field === 'allowed_file_upload_types' || field === 'bank_details') {
          try {
