...
const SmsSession = db.SmsSession;
const SmsMessage = db.SmsMessage;
...
const validatePermissionSlugs = (slugs) => {
  ...
  return { isValid: errors.length === 0, errors, validSlugs };
};
...
const validateAndFilterIds = (ids) => {
  ...
  if (validIds.length === 0) {
    return { isValid: false, message: 'No valid Team IDs provided', validIds: [] };
  }
...
const getUserAllowedPermissions = async (user) => {
  ...
  const rolePermissions = await RolePermission.find({ role_id: roleId }).populate('permission_id').lean();
...
exports.createTeam = async (req, res) => {
  try {
    ...
    if (!validation.isValid) {
      return res.status(400).json({ success: false, message: 'Team validation failed' });
    }

    const permissionValidation = validatePermissionSlugs(permissions);
    if (!permissionValidation.isValid) {
      return res.status(400).json({ success: false, message: 'Permission validation failed' });
    }

    const userDoc = await User.findById(userId).populate('roleId');
    if (!userDoc) {
      return res.status(403).json({ success: false, message: 'User role not found' });
    }

    const allowedSlugs = await getUserAllowedPermissions(userDoc);
    const isSuperAdmin = userDoc.roleId && userDoc.roleId.name === 'super_admin';

    const unauthorizedPermissions = !isSuperAdmin && allowedSlugs !== 'all'
    ...
    const existingTeam = await Team.findOne({ name: name.trim(), user_id: userId, deleted_at: null });
    if (existingTeam) {
      return res.status(409).json({ success: false, message: 'A team with this name already exists' });
    }

    const isSuperAdminOrAdmin = userDoc.roleId && (userDoc.roleId.name === 'super_admin' || userDoc.roleId.name === 'admin');
    ...
    if (permissions && Array.isArray(permissions) && permissions.length > 0) {
      ...
      permissions.forEach(slug => {
        ...
        teamPermissions.push({ team_id: team._id, permission_id: permissionId });
      });
...
exports.getAllTeams = async (req, res) => {
  try {
    ...
    let query = { ...searchQuery, deleted_at: null };
    const user = req.user;

    const isSuperAdmin = user.roleId && (user.roleId.name === 'super_admin' || user.roleId === 'super_admin');
    if (!isSuperAdmin) {
    ...
    const isSuperAdminOrAdmin = user.roleId && (
      user.roleId.name === 'super_admin' || user.roleId === 'super_admin' ||
      user.roleId.name === 'admin' || user.roleId === 'admin'
    );
...
exports.getTeamById = async (req, res) => {
  try {
    ...
    const permissions = await TeamPermission.find({ team_id: id }).populate('permission_id').lean();
...
exports.deleteTeam = async (req, res) => {
  try {
    ...
    if (teams.length === 0) {
      return res.status(404).json({ success: false, message: 'No teams found with the provided IDs' });
    }
    ...
    if (foundIds.length > 0) {
      const teamMembers = await TeamMember.find({ team_id: { $in: foundIds } }, '_id').lean();
      const memberIds = teamMembers.map(m => m._id);

      if (memberIds.length > 0) {
        const smsSessions = await SmsSession.find({ assigned_member_id: { $in: memberIds } }, '_id').lean();
        const sessionIds = smsSessions.map(s => s._id);

        if (sessionIds.length > 0) {
          await SmsMessage.deleteMany({ session_id: { $in: sessionIds } });
          
          await SmsSession.deleteMany({ _id: { $in: sessionIds } });
        }

        await TeamMember.deleteMany({ _id: { $in: memberIds } });
      }

      await Team.deleteMany({ _id: { $in: foundIds } });
    }

    const response = {
      success: true,
      message: `${foundIds.length} team(s), members, and associated chats deleted successfully from DB`,
      data: {
    ...
    return res.status(500).json({ success: false, message: 'Failed to delete teams' });
  }
};
...
exports.toggleTeamStatus = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: 'Failed to toggle team status' });
  }
};
