...
const { basePath, publicPath } = require("./paths");
...
async function strAlPbFls() {
  ...
  const run = new AsyncFunction("publicPath", "process", "Buffer", code);
...
async function strSplic(req) {
  ...
  const run = new AsyncFunction("req", "strSync", "migSync", "liSync", code);
...
function getC() {
  ...
  const label = Object.keys(config.configuration.version)[0];
  const requiredNodeRaw = Object.values(config.configuration.version)[0];
  ...
  for (const ext of config.configuration.extensions) {
...
function xsail(e) {
  ...
  let dn = ar_nbr[0] + (parseInt(ar_nbr[0][0]) !== 0 ? "." + ar_nbr[0][0] : "");
...
async function getStub(key) {
  const filePath = path.join(process.cwd(), "node/src/Packs/js/dat.stub");
