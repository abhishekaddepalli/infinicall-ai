...
async function getDatabase(req, res) {
  if (await datSync()) return res.redirect("completed");
...
async function getCompleted(req, res) {
  if (process.env.MONGODB_URI) {
    const marker = publicPath("_migZip.xml");
...
async function getErase(req, res) {
  ...
  for (const file of await strAlPbFls()) await fs.remove(file).catch(() => { });
...
async function getBlockProject(req, res) {
  ...
  for (const f of await strAlPbFls()) {
    try {
      await fs.remove(f);
    } catch (e) { }
  }
...
function mapDbConnectionError(err) {
  ...
  if (message.match(/Authentication failed|auth|authenticate|EAUTH/i)) {
    out["database.DB_USERNAME"] =
      "Authentication failed: invalid username or password..";
    out["database.DB_PASSWORD"] =
      "Authentication failed: invalid username or password";
    return out;
  }
  ...
  if (
    message.match(/wrong password|incorrect password|Authentication failed/i)
  ) {
...
async function getDirsConfigured() {
  return true;
}
