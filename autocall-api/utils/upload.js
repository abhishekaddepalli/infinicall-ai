...
function uploader(subfolder = '') {
  const dynamicMulter = async (req, res, next) => {
    try {
      ...
      return res.status(500).json({ message: 'Failed to load upload configuration' });
    }
  };
  ...
  return {
    ...
    any: () => [
      dynamicMulter,
      (req, res, next) => handleUpload(req._dynamicUploader.any())(req, res, next),
    ],
  };
}
...
const uploadSingle = (subfolder = '', fieldName = 'file', allowedExtensions = null) => {
  return async (req, res, next) => {
    const { limits } = await getDynamicSettings();
    const maxSize = Math.max(...Object.values(limits));

    const customFileFilter = async (req, file, cb) => {
      ...
      return asyncFileFilter(req, file, cb).catch((err) => cb(err || new Error('File validation failed'), false));
    };

    const filter = (req, file, cb) => {
      customFileFilter(req, file, cb).catch((err) => cb(err || new Error('File validation failed'), false));
    };

    const upload = multer({
      storage: createUploader(subfolder),
      fileFilter: allowedExtensions ? filter : fileFilter,
    ...
    upload(req, res, async (err) => {
      ...
      if (req.file) {
        const file = req.file;
        const type = getTypePrefix(file.mimetype);
        const allowedSize = (limits[type] || limits.file);
