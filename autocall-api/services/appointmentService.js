...
class AppointmentService {
  ...
  async updateGoogleSheetRow(userId, appointment, spreadsheetId = null, sheetName = null, range = null) {
    try {
      ...
      // 1. Fetch all rows to find the appointment
      ...
      // 2. Find the row index matching the phone number
      let rowIndexToUpdate = -1;
      for (let i = rows.length - 1; i >= 0; i--) {
        const row = rows[i] || [];
        // We match by phone since it's unique to the user, searching from the bottom up to get the latest
