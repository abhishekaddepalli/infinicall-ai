...
class WebRTCManager extends EventEmitter {
    ...
    async answerCall(waCallId, phoneNumberId, sdpOffer, agent, contact, callLog) {
        ...
        if (agent && agent.enable_call_recording) {
            const recordingsDir = path.join(process.cwd(), 'uploads', 'recordings');
            if (!fs.existsSync(recordingsDir)) fs.mkdirSync(recordingsDir, { recursive: true });

            const rawFilePath = path.join(recordingsDir, `wa_${waCallId}.raw`);
            const stream = fs.createWriteStream(rawFilePath);
            callRecorders.set(waCallId, stream);

            if (callLog) {
                callLog.recording_url = `/uploads/recordings/wa_${waCallId}.wav`;
                await callLog.save();
            }
        }
    ...
    async _processSpeech(waCallId, agent, callLog, opusFrames) {
        ...
        try {
            ...
            if (agent.enable_call_recording) {
                ...
                const recorder = callRecorders.get(waCallId);
                if (recorder) {
                    recorder.write(pcmBuffer);
                }
            }

            const callAutomationService = require('./whatsappCallAutomationService');
    ...
    async playAudio(waCallId, pcmBuffer) {
        ...
        const pcmInput = new Int16Array(pcmBuffer.buffer, pcmBuffer.byteOffset, pcmBuffer.length / 2);
        ...
        const recorder = callRecorders.get(waCallId);
        if (recorder) {
            const buf = Buffer.alloc(pcm8k.length * 2);
            for (let i = 0; i < pcm8k.length; i++) {
                buf.writeInt16LE(pcm8k[i], i * 2);
            }
            recorder.write(buf);
        }
    ...
    cleanup(waCallId) {
        ...
        if (recorder) {
            recorder.end();
            callRecorders.delete(waCallId);
            try {
                const rawFilePath = path.join(process.cwd(), 'uploads', 'recordings', `wa_${waCallId}.raw`);
                const wavFilePath = path.join(process.cwd(), 'uploads', 'recordings', `wa_${waCallId}.wav`);
                if (fs.existsSync(rawFilePath)) {
                    const rawData = fs.readFileSync(rawFilePath);
                    const wavBuffer = this.generateWavBuffer(rawData, 8000, 1, 16);
                    fs.writeFileSync(wavFilePath, wavBuffer);
                    fs.unlinkSync(rawFilePath);
                    console.log(`[WebRTC] Call recording saved to ${wavFilePath}`);
                }
            } catch (err) {
                console.error('[WebRTC] Error processing final call recording:', err);
            }
        }
    ...
    async waitForConnection(waCallId, timeout = 10000) {
        ...
        while (Date.now() - startTime < timeout) {
            const conn = this.connections.get(waCallId);
            if (conn && conn.pc) {
                if (conn.pc.connectionState === 'connected') return true;
                if (conn.pc.connectionState === 'failed' || conn.pc.connectionState === 'closed') return false;
            }
    ...
    async initiateOutboundCall(phoneNumberId, contactNumber, agent, contact) {
        try {
            ...
            return { sdp: localSdp, pc, audioSource };
        } catch (error) {
            console.error('[WebRTC] Error in initiateOutboundCall:', error);
            throw error;
        }
    }
}
