        ...
        for (const row of uniqueContacts) {
          ...
          try {
            if (agent.telephony_provider === 'meta_whatsapp' || phoneNumberDoc.type === 'whatsapp') {
              ...
              successCount++;
            } else if (agent.telephony_provider?.includes('plivo') || phoneNumberDoc.provider === 'plivo') {
              const answerUrl = `${appUrl}/api/calls/plivo-xml?flowId=${flowId}&userId=${campaign.userId}&agentId=${campaign.agentId}`;
              ...
              const callSid = plivoCall.request_id || plivoCall.call_uuid || `plivo_${Date.now()}`;
              ...
              await new Promise(resolve => setTimeout(resolve, 3000));
              console.log(`Plivo Call to ${to} initiated with ID: ${callSid}`);
              successCount++;
            } else {
        ...
        for (const row of contactsToProcess) {
          ...
          try {
            ...
            await SMSLog.create({
              ...
              status: 'sent',
              twilio_message_sid: sentMessage ? sentMessage.sid : null
            });
