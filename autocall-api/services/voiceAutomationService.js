    ...
    ws.on('message', async (message) => {
      ...
      switch (msg.event) {
        case 'start':
          ...
          flowId = customData.flowId || null;
          agentId = customData.agentId || null;
          ...
          console.log(`Stream started: CallSid=${callSid}, FlowId=${flowId}, AgentId=${agentId}`);
          try {
            const callLog = await Call.findOne({ twilio_call_sid: callSid });
            const userSettings = await UserSettings.findOne({ user: userId }).populate('ai_model');
...
<Response>
  ...
  async handleCompleteSpeech(streamSid, ws, audioBuffer) {
    ...
    const text = await this.processSTT(audioBuffer, stream.userId);
