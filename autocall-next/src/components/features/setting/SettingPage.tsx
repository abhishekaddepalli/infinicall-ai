...
import { Cpu, MessageSquare, Phone, Shield } from 'lucide-react';
...
const SettingPage = () => {
  ...
  const currentValues = useMemo(() => {
    ...
    return {
      ...
      sarvam_api_key: user.sarvam_api_key || '',
      ...
      plivo_phone_number: user.plivo_phone_number || '',
  ...
  const onSubmit = async (values: typeof initialValues, { resetForm }: any) => {
    try {
      const userValues = {
        ...
        sarvam_api_key: values.sarvam_api_key,
        ...
        plivo_phone_number: values.plivo_phone_number,
  ...
  const TABS = [
    { id: 'ai', label: 'AI & Voice Providers', icon: Cpu, description: 'OpenAI, Sarvam AI & ElevenLabs' },
    { id: 'twilio', label: 'Telephony (Twilio & Plivo)', icon: Phone, description: 'Twilio & Plivo Configuration' },
                    ...
                    {TABS.map(({ id, icon: Icon, label }) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span>{t(id === "ai" ? "settings_tabs_ai" : id === "twilio" ? "settings_tabs_twilio" : "settings_tabs_waba", label)}</span>
                        </Button>
                      );
                    })}
                  </div>
                    ...
                    return (
                      <Button key={id} type="button" onClick={() => setActiveTab(id)} className={`w-full flex items-center gap-3 py-3 px-3 rounded-radius text-left group shadow-none h-auto ${isActive ? "bg-primary text-white dark:text-zinc-50 font-semibold" : "text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-800/30 hover:text-slate-900 dark:hover:text-zinc-100 font-medium bg-transparent"}`}>
                        ...
                        <div className="flex-1 min-w-0 text-left rtl:text-right">
                          <p className="text-md font-bold leading-tight">{t(id === "ai" ? "settings_tabs_ai" : id === "twilio" ? "settings_tabs_twilio" : "settings_tabs_waba", label)}</p>
                          <p className={cn("text-xs lg:text-sm truncate text-subtitle-color  hidden lg:block", isActive ? "text-white/80" : "text-subtitle-color")}>{t(id === "ai" ? "settings_tabs_ai_desc" : id === "twilio" ? "settings_tabs_twilio_desc" : "settings_tabs_waba_desc", description)}</p>
                        </div>
                      </Button>
                    );
                  })}
