...
const PhoneNumberPage = () => {
  ...
  const columns: Column<PhoneNumber>[] = [
    ...
    {
      header: t('ai_voice_sync', 'AI & Voice Sync'),
      cell: (row) => (
        <div className="flex flex-wrap items-center gap-1.5 max-w-[260px]">
          {/* ElevenLabs Status */}
          {row.is_synced_to_elevenlabs ? (
            <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 dark:text-emerald-400 font-extrabold text-[10px] gap-1 px-2.5 py-1 rounded-full">
            ...
            canUpdatePhone && (
              <Button
                ...
                className="h-7 rounded-full bg-primary/10 text-primary dark:bg-white/5 border-input-border-color text-xs font-black gap-1.5 hover:bg-primary hover:text-white transition-all px-2.5"
                title={t('sync_elevenlabs', 'Sync to ElevenLabs')}
              >
                <RefreshCw className={cn('w-3 h-3', isSyncingElevenLabs && syncingRowId === (row._id || row.id) && 'animate-spin')} />
                <span>Sync ElevenLabs</span>
              </Button>
            )
          )}

          {/* Vobiz AI Engine Badge */}
          {row.provider === 'vobiz' && (
            <Badge variant="outline" className="bg-orange-500/10 text-orange-600 border-orange-500/20 dark:text-orange-400 font-extrabold text-[10px] gap-1 px-2.5 py-1 rounded-full">
              <Zap className="w-3 h-3 text-orange-500 fill-orange-500/20" />
              <span>Vobiz AI</span>
            </Badge>
          )}

          {/* Sarvam AI Ready Badge */}
          <Badge variant="outline" className="bg-purple-500/10 text-purple-600 border-purple-500/20 dark:text-purple-400 font-extrabold text-[10px] gap-1 px-2.5 py-1 rounded-full">
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) => (
        <div className="font-extrabold text-sm text-title flex items-center gap-0.5">
          <span className="text-primary font-bold">₹</span>
          <span>{row.purchase_price ? row.purchase_price.toFixed(2) : '0.00'}</span>
        </div>
      ),
    },
    {
        ...
        <div className="flex items-center gap-2">
          {row.provider ? (
            <Badge variant="outline" className={cn(
              "font-black uppercase tracking-wider text-[10px] px-2.5 py-1 rounded-full",
              row.provider === 'vobiz' && "bg-orange-500/10 text-orange-600 border-orange-500/30 dark:text-orange-400",
              row.provider === 'twilio' && "bg-red-500/10 text-red-600 border-red-500/30 dark:text-red-400",
              row.provider === 'plivo' && "bg-blue-500/10 text-blue-600 border-blue-500/30 dark:text-blue-400",
              row.provider === 'sip' && "bg-emerald-500/10 text-emerald-600 border-emerald-500/30 dark:text-emerald-400",
              !['vobiz', 'twilio', 'plivo', 'sip'].includes(row.provider) && "bg-slate-100 text-slate-600 border-slate-200 dark:bg-zinc-800 dark:text-zinc-400"
            )}>
              {row.provider}
            </Badge>
          ) : (
