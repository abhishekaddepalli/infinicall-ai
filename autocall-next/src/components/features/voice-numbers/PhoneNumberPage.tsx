...
const PhoneNumberPage = () => {
  ...
  const handleLoadTwilio = async () => {
    try {
      ...
      toast.error(error?.data?.message || t('twilio_sync_failed'))
    }
  }
  ...
  const handleSyncElevenLabs = async (num: PhoneNumber) => {
    try {
      ...
      toast.error(error?.data?.message || t('sync_failed'))
    } finally {
  ...
  const handleSipImport = async (values: {
    ...
    try {
      ...
      toast.error(error?.data?.message || t('sip_import_failed'))
      throw error
    }
  }
  ...
  const handleAssignSip = async ({ sip_trunk_id }: { sip_trunk_id: string }) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || t('assign_sip_failed'))
      throw error
    }
  }
  ...
  const handleAssignPrice = async ({ purchase_price, validity_days }: { purchase_price: number; validity_days: number }) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || t('assign_price_failed'))
      throw error
    }
  }
  ...
  const handleAssignAgent = async ({ agent_id }: { agent_id: string | null }) => {
    ...
    try {
      ...
      toast.error(
        error?.data?.message || t('assign_agent_failed'),
      )
      throw error
    }
  }
    ...
    },
    {
      ...
      cell: (row) => (
        ...
        >
          ...
          <span className="text-[10px] font-black uppercase tracking-tighter">{t(row.status)}</span>
        </div>
      ),
    },
    {
