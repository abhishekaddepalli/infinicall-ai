...
export default function CampaignHistoryPage() {
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      className: 'lg991:min-w-[250px]',
      cell: (row) => {
        if (!row.recording_url) return <span className="text-xs text-zinc-400 italic font-medium">—</span>
        return (
        ...
        <div className="flex gap-2">
          {row.transcript ? (
            <Button
              ...
              className="h-8 rounded-lg flex items-center gap-1.5 text-xs font-bold bg-primary/5 hover:bg-primary border-primary/20 text-primary hover:text-white"
            >
      ...
      <Dialog open={!!activeTranscript} onOpenChange={(open) => { if (!open) setActiveTranscript(null) }}>
        <DialogContent className="max-w-[600px] max-h-[80vh] gap-0! overflow-y-auto no-scrollbar bg-bg-card border-none shadow-2xl rounded-lg sm:p-6 p-4">
          ...
          <div className="space-y-4 pt-4">
            <div className="bg-subcard p-4 rounded-lg border border-input-border-color! max-h-[50vh] overflow-y-auto no-scrollbar">
              {activeTranscript && (
                  ...
                  <div className="space-y-4">
                    {activeTranscript.map((tItem: any, idx: number) => (
                      <div key={idx} className={`flex flex-col ${tItem.role === 'user' ? 'items-end' : 'items-start'}`}>
                        <span className={`text-[10px] font-extrabold uppercase tracking-wider mb-1 px-1 ${tItem.role === 'user' ? 'text-primary' : 'text-slate-400 dark:text-zinc-500'}`}>
                          {tItem.role === 'user' ? t('user') : t('agent')}
                        </span>
                        <div className={`p-3 rounded-2xl max-w-[85%] text-sm font-semibold shadow-sm leading-relaxed ${tItem.role === 'user' ? 'bg-primary/10 text-primary rounded-tr-none border border-primary/15' : 'bg-bg-card text-slate-800 dark:text-slate-200 rounded-tl-none border border-input-border-color'}`}>
                          {tItem.text}
                        </div>
                      </div>
                    ))}
