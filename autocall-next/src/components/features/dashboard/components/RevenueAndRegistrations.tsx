        ...
        <motion.div variants={cardVariants} className="sm:p-6 p-4 rounded-radius border border-input-border-color bg-bg-card  space-y-6 overflow-hidden justify-between h-full">
          <div>
            <div className="flex items-center justify-between pb-4">
              ...
              <Link href={ROUTES.MEMBERS} className="text-md font-bold text-subtitle-color cursor-pointer">
                {t('see_all', 'See All')}
              </Link>
            </div>
                ...
                return (
                  <div key={i} className="flex items-center justify-between p-3 rounded-lg border border-input-border-color bg-subcard transition-all">
                    <div className="flex items-center gap-3 min-w-0">
                      {usr.avatar ? <Image src={avatarSrc} alt={usr.name || t('user_image')} width={32} height={32} unoptimized  className="w-10 h-10 rounded-lg object-cover flex-shrink-0" /> : <div className="w-10 h-10 rounded-lg bg-primary/10 text-primary font-bold text-xs flex items-center justify-center uppercase flex-shrink-0">{usr.name?.slice(0, 2) || t('us')}</div>}
