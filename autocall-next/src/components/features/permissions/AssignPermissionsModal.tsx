    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-xl! max-w-[calc(100%-2rem)]! max-h-[90vh] overflow-hidden flex flex-col border-none shadow-2xl bg-light-body border! rounded-border-radius!">
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 glass-card glass-dark-card gap-4 p-4 rounded-border-radius bg-card-color border border-glass-border">
                  <div className="space-y-1.5 flex flex-col">
                    <Label className="text-md font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-foreground mb-2!">
                      {t('module')}
                    </Label>
                    ...
                    <div className="grid grid-cols-1 gap-2 max-h-55 overflow-y-auto custom-scrollbar">
                      {Object.entries(assignedPermissions).map(([subId, access]) => {
                        ...
                        return (
                          <div
                            key={subId}
                            className="flex items-center justify-between p-3 pl-4 rounded-border-radius glass-dark-card bg-bg-card border border-glass-border group hover:border-primary transition-all"
                          >
                            <div className="flex flex-col">
                              <span className="text-text-lg font-bold">{sub?.name || t('unknown')}</span>
          ...
          <DialogFooter className="pt-2 gap-3 sm:gap-4 mt-2 p-6 pt-0!">
            <Button
              type="button"
              variant="outline"
              onClick={onClose}
