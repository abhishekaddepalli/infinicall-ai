...
import PageForm from '@/components/features/app-setting/WebPageForm'
...
import { ApiError, Page } from '@/types/api'
import { ViewType } from '@/types/pages'
...
export default function SettingsPagesPage() {
  ...
  const canManage = hasPermission(PERMISSIONS.UPDATE_PAGES)
  ...
  const columns: Column<Page>[] = [
    {
      ...
      cell: (row) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-primary/5 text-primary dark:bg-light-button flex items-center justify-center">
            <FileText className="h-4 w-4" />
          </div>
          <span className="font-medium text-md break-all whitespace-normal line-clamp-2" title={row.title}>
            {row.title}
          </span>
        </div>
      ),
    },
    {
      ...
      cell: (row) => (
        <Badge
          variant="outline"
          className=" text-xs bg-primary/10 border-primary/5 rounded-lg px-3 py-1 text-primary whitespace-nowrap"
        >
          /{row.slug}
        </Badge>
      ),
    },
    {
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          {canManage ? (
            <Switch checked={row.status} onCheckedChange={() => handleStatusChange(row._id || row.id)} className='bg-switch-background dark:bg-switch-background' />
          ) : (
            <Switch checked={row.status} disabled className='bg-switch-background dark:bg-switch-background' />
          )}
        </div>
      ),
    },
    {
      ...
      className: 'min-w-[150px]',
      cell: (row) => (
        <span className="text-subtitle-color text-md font-medium! whitespace-nowrap">
  ...
  return (
    <>
      <TableLayout
        ...
        isLoading={isLoading}
        loadingText={t("loading_pages", "LOADING PAGES...")}
        emptyStateTitle={t("no_pages_title", "No Pages Found")}
        emptyMessage={t("no_pages_desc", "Build and publish new pages to expand your website's content.")}
        emptyStateActionLabel={t('create_page')}
