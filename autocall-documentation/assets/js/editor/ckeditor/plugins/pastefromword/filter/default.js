  ...
  CKEDITOR.cleanWord = function (a, b) {
    ...
    var g = CKEDITOR.htmlParser.fragment.fromHtml(a),
      h = {
        root: function (a) {
          a.filterChildren(p);
          CKEDITOR.plugins.pastefromword.lists.cleanup(f.createLists(a));
        },
        elementNames: [
          [/^\?xml:namespace$/, ""],
          [/^v:shapetype/, ""],
        ...
        elements: {
          ...
          td: function (a) {
            ...
            var c = m.parseCssText(a.attributes.style),
              h;
            for (h in c) ((g = c[h]), delete c[h], (c[h.toLowerCase()] = g));
            for (h = 0; h < e.length; h++)
              c[e[h]] && ((g = c[e[h]]), (d[g] = d[g] ? d[g] + 1 : 1));
            k.createStyleStack(
              a,
              p,
              b,
  ...
  CKEDITOR.plugins.pastefromword.styles = {
    ...
    sortStyles: function (a) {
      for (
        ...
        -1 !== m.indexOf(b, d[h].toLowerCase()) ? e.push(d[h]) : g.push(d[h]);
      e.sort(function (a, c) {
    ...
    inliner: {
      ...
      sort: function (a) {
        return a.sort(
          (function (a) {
            ...
            return function (a, b) {
              var g = -1 !== ("" + a.selector).indexOf(".") ? 1 : 0,
                g = (-1 !== ("" + b.selector).indexOf(".") ? 1 : 0) - g;
              return 0 !== g
                ? g
      ...
      inline: function (a) {
        ...
        CKEDITOR.tools.array.forEach(c, function (a) {
          ...
          for (q = 0; q < a.count(); q++)
            ((c = a.getItem(q)),
              (f = CKEDITOR.tools.parseCssText(c.getAttribute("style"))),
              (f = CKEDITOR.tools.extend({}, f, b)),
              c.setAttribute("style", CKEDITOR.tools.writeCssText(f)));
        });
        return d;
      },
    },
  ...
  CKEDITOR.plugins.pastefromword.lists = {
    ...
    setListStart: function (a) {
      ...
      switch (a.attributes["cke-list-style-type"]) {
        case "lower-roman":
        case "upper-roman":
          a.attributes.start = f.toArabic(f.getSubsectionSymbol(b[c])) - c;
          break;
        case "lower-alpha":
        case "upper-alpha":
          a.attributes.start =
      ...
      for (a = 0; a < g.length; a++) {
        ...
        for (d = 0; d < h.length; d++) {
          ...
          for (c = b.attributes["cke-list-level"]; c > l.length; ) {
            ...
            n.push(r);
            k = r;
            c == l.length && f.setListSymbol(r, b.attributes["cke-symbol"], c);
          }
          for (; c < l.length; )
            (l.pop(),
              (k = l[l.length - 1]),
              c == l.length &&
                f.setListSymbol(k, b.attributes["cke-symbol"], c));
          b.remove();
          k.add(b);
        }
        l[0].children.length &&
        ...
        for (d = 0; d < h.length; d++) this.determineListItemValue(h[d]);
      }
      return e;
    },
    cleanup: function (a) {
    ...
    groupLists: function (a) {
      ...
      for (b = 1; b < a.length; b++) {
        ...
        c.attributes["cke-indentation"] =
          c.attributes["cke-indentation"] || f.getElementIndentation(c);
        c.previous !== g && (f.chopDiscontinuousLists(e, d), d.push((e = [])));
        e.push(c);
      }
    ...
    chopDiscontinuousLists: function (a, b) {
      for (var c = {}, d = [[]], e, g = 0; g < a.length; g++) {
        var h = c[a[g].attributes["cke-list-level"]],
          ...
          n;
        h
          ? ((n = h.type.match(/alpha/) && 7 == h.index ? "alpha" : n),
            (n =
            ...
            (l = this.getListItemInfo(a[g])),
            (h.type != k.type ||
              (e && l.id != e.id && !this.isAListContinuation(a[g]))) &&
              d.push([]))
