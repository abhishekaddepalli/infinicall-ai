    ...
    /******/ [
      ...
      /***/ function (module, exports, __webpack_require__) {
        ...
        !((__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
          ...
          function kMeans(data, k, dataMeta) {
            ...
            var centroids = createRandCent(
              k,
              calcExtents(data, dataMeta.dimensions),
            );
          ...
          function hierarchicalKMeans(data, clusterCountOrConfig, stepByStep) {
            ...
            if (isOutputTypeSingle) {
              outputSingleData = [];
              var outputClusterIndexDimension =
                dataMeta.outputClusterIndexDimension;

              setClusterIndex = function (dataIndex, clusterIndex) {
                outputSingleData[dataIndex][outputClusterIndexDimension] =
                  clusterIndex;
              };
            ...
            function oneStep() {
              ...
              if (index < k) {
                ...
                for (
                  var i = 0, j = 0;
                  i < dataSet.length && j < newClusterAss.length;
                  i++
                ) {
          ...
          function setCentroidToResultData(result, dataMeta) {
            ...
            if (
              dataMeta.outputType !== OutputType.SINGLE ||
              outputCentroidDimensions == null
            ) {
              return;
            }
            ...
            for (var i = 0; i < outputSingleData.length; i++) {
              ...
              var dimLen = Math.min(
                centroid.length,
                outputCentroidDimensions.length,
              );
          ...
          function createRandCent(k, extents) {
            ...
            for (var j = 0; j < extents.length; j++) {
              ...
              for (var i = 0; i < k; i++) {
                centroids[i][j] =
                  extentItem.min + extentItem.span * Math.random();
              }
            }
            return centroids;
          }
          ...
          function parseDataMeta(dataSet, config) {
            ...
            var dimensions = normalizeDimensions(
              config.dimensions,
              defaultDimensions,
            );
            ...
            if (
              ...
              !numberUtil.isNumber(outputClusterIndexDimension)
            ) {
              throw new Error(
          ...
          var regreMethods = {
            ...
            exponential: function (predata, opt) {
              ...
              for (var i = 0; i < predata.length; i++) {
                ...
                sumXY += rawItem[xDimIdx] * rawItem[yDimIdx];
                sumXXY +=
                  rawItem[xDimIdx] * rawItem[xDimIdx] * rawItem[yDimIdx];
                sumYlny += rawItem[yDimIdx] * Math.log(rawItem[yDimIdx]);
                sumXYlny +=
                  rawItem[xDimIdx] *
                  rawItem[yDimIdx] *
                  Math.log(rawItem[yDimIdx]);
              }

              var denominator = sumY * sumXXY - sumXY * sumXY;
              var coefficient = Math.pow(
                Math.E,
                (sumXXY * sumYlny - sumXY * sumXYlny) / denominator,
              );
              ...
              for (var j = 0; j < predata.length; j++) {
                ...
                resultItem[xDimIdx] = rawItem[xDimIdx];
                resultItem[yDimIdx] =
                  coefficient * Math.pow(Math.E, index * rawItem[xDimIdx]);
                result.push(resultItem);
              }
            ...
            logarithmic: function (predata, opt) {
              ...
              var gradient =
                (i * sumYlnx - sumY * sumlnx) /
                (i * sumlnxlnx - sumlnx * sumlnx);
              ...
              for (var j = 0; j < predata.length; j++) {
                ...
                resultItem[xDimIdx] = rawItem[xDimIdx];
                resultItem[yDimIdx] =
                  gradient * Math.log(rawItem[xDimIdx]) + intercept;
                result.push(resultItem);
              }
            ...
            polynomial: function (predata, opt) {
              ...
              for (var i = coeArray.length - 1; i >= 0; i--) {
                if (i > 1) {
                  expression +=
                    Math.round(coeArray[i] * Math.pow(10, i + 1)) /
                      Math.pow(10, i + 1) +
                    "x^" +
                    i +
                    " + ";
                  ...
                  expression +=
                    Math.round(coeArray[i] * 100) / 100 + "x" + " + ";
                } else {
        ...
        !((__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
          ...
          function computeBins(data, optOrMethod) {
            ...
            for (var i = 0; i < values.length; i++) {
              if (minValue <= values[i] && values[i] <= maxValue) {
                bins[bisect(rangeArray, values[i], 0, len)].sample.push(
                  values[i],
                );
              }
            }
          ...
          var thresholdMethod = {
            ...
            scott: function (data, min, max) {
              return Math.ceil(
                (max - min) /
                  (3.5 * deviation(data) * Math.pow(data.length, -1 / 3)),
              );
            },
      ...
      /***/ function (module, exports, __webpack_require__) {
        ...
        !((__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
          ...
          return function (start, stop, count) {
            ...
            var resultStep = +(stop >= start ? step1 : -step1).toFixed(
              toFixedPrecision,
            );
          ...
          return {
            ...
            transform: function transform(params) {
              ...
              var result = regression(method, upstream.cloneRawData(), {
                order: config.order,
                dimensions: transformHelper.normalizeExistingDimensions(
                  params,
                  config.dimensions,
                ),
              });
      ...
      /***/ function (module, exports, __webpack_require__) {
        ...
        !((__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
           ...
           */
          function normalizeExistingDimensions(
            transformParams,
            dimensionsConfig,
          ) {
          ...
          function normalizeNewDimensions(dimensionsConfig) {
            ...
            function parseDimensionNewItem(dimConfig) {
              if (numberUtil.isNumber(dimConfig)) {
                ...
                objectUtil.isObject(dimConfig) &&
                numberUtil.isNumber(dimConfig.index)
              ) {
                return dimConfig;
              }
              throw new Error(
        ...
        !((__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
          ...
          return {
            ...
            transform: function transform(params) {
              ...
              var outputClusterIndexDimension =
                transformHelper.normalizeNewDimensions(
                  config.outputClusterIndexDimension,
                );
              var outputCentroidDimensions =
                transformHelper.normalizeNewDimensions(
              ...
              var result = clustering.hierarchicalKMeans(
                upstream.cloneRawData(),
                {
                  ...
                  outputCentroidDimensions: (outputCentroidDimensions || {})
                    .index,
                },
              );
              ...
              resultDimsDef[outputClusterIndexDimension.index] =
                outputClusterIndexDimension.name;

              if (outputCentroidDimensions) {
                for (
                  var i = 0;
                  i < outputCentroidDimensions.index.length;
                  i++
                ) {
                  if (outputCentroidDimensions.name[i] != null) {
                    resultDimsDef[outputCentroidDimensions.index[i]] =
                      outputCentroidDimensions.name[i];
                  }
                }
              }
