...
(function (context) {
    ...
    if (buttons.length) {
      for (var i = 0; i < buttons.length; i++) {
        ...
        if (btnDefine) {
          ...
          btn.innerHTML = testHelper.encodeHTML(
            btnDefine.name || btnDefine.text || "button",
          );
  ...
  testHelper.printAssert = function (chartOrDomId, checkerFn) {
    ...
    var printAssertRecord =
      hostDOMEl.__printAssertRecord || (hostDOMEl.__printAssertRecord = []);
    ...
    function relayoutResult() {
      ...
      var lineHeight = Math.min(
        fontSize + 10,
        (chartHeight - 20) / printAssertRecord.length,
      );
  ...
  var printObject = (testHelper.printObject = function (obj, opt) {
    opt = typeof opt === "string" ? { key: opt } : opt || {};
    ...
    function doPrint(obj, key, depth) {
      ...
      switch (objType) {
        ...
        case "typedArray":
          hasLineBreak =
            opt.arrayLineBreak != null ? opt.arrayLineBreak : false;
          ...
          var delimiter =
            "," + (hasLineBreak ? lineBreak + subCodeIndent : " ");
        ...
        case "object":
          hasLineBreak =
            opt.objectLineBreak != null ? opt.objectLineBreak : true;
          var childBuilder = [];
          for (var i in obj) {
      ...
      for (var i = 0; i < data.length && i <= dataTableLimit; i++) {
        ...
        for (var key in line) {
          if (line.hasOwnProperty(key)) {
            ...
            htmlLine.push(
              '<td class="test-data-table-key">' +
                testHelper.encodeHTML(keyText) +
                "</td>",
            );
  ...
  function createObjectHTML(obj, key) {
    ...
    return ['<pre class="test-print-object">', html, "</pre>"].join("");
  }

  var getDom = (testHelper.getDom = function (domOrId) {
    return getType(domOrId) === "string"
      ? document.getElementById(domOrId)
      : domOrId;
  });
