...
# ==============================================================================
# Autocall Automated Installation Script (Multi-Project Version)
...
echo -e "${GREEN}====================================================${NC}"
echo -e "${GREEN}      Autocall Multi-Project Installation Script        ${NC}"
echo -e "${GREEN}====================================================${NC}"
...
echo -e "\n${YELLOW}--- Selection ---${NC}"
read -p "Install Autocall Backend? (y/n): " INSTALL_BACKEND
read -p "Install Autocall Frontend? (y/n): " INSTALL_FRONTEND


if [[ "$INSTALL_BACKEND" == "y" ]]; then
    read -p "Enter Autocall BACKEND Git URL: " BACKEND_REPO
fi
if [[ "$INSTALL_FRONTEND" == "y" ]]; then
    read -p "Enter Autocall FRONTEND Git URL: " FRONTEND_REPO
fi
...
if [[ "$IS_PRIVATE" == "y" ]]; then
    if [ ! -f ~/.ssh/id_rsa.pub ] && [ ! -f ~/.ssh/id_ed25519.pub ]; then
        ...
        ssh-keygen -t ed25519 -C "autocall-server" -f ~/.ssh/id_ed25519 -N ""
...
setup_project() {
    ...
    if [[ "$TYPE" == "backend" ]]; then
        sed -i "s|MONGODB_URI=.*|MONGODB_URI=mongodb://$MONGO_USER:$MONGO_PASS@127.0.0.1:27017/autocall?authSource=admin|g" .env
...
[[ "$INSTALL_BACKEND" == "y" ]] && setup_project "Autocall-backend" "$BACKEND_REPO" "backend"
[[ "$INSTALL_FRONTEND" == "y" ]] && setup_project "Autocall-frontend" "$FRONTEND_REPO" "frontend"
